# -*- coding: utf-8 -*-
""" 
Logistic Regression without primary biomarkers.
Focus: Discovery of alternative biomarkers for CKD prediction using a blinded linear model.
"""
...
# Step 3: Preparing data for Machine Learning while preventing data leaks.
columnas_excluir = [
    'ID_Participante', 
    'Target_Estadio', 
    'eGFR_Real', 
...
print("\n🛠️ Filling missing data and scaling secondary biomarkers...")

# A. Imputation of missing values ​​(we use direct matrices for computational efficiency)
imputador = SimpleImputer(strategy='median')
X_train_imputed = imputador.fit_transform(X_train)
X_test_imputed = imputador.transform(X_test)

# B. Feature scaling (indispensable for the stability of linear models).
scaler = StandardScaler()
X_train_scaled = scaler.fit_transform(X_train_imputed)
X_test_scaled = scaler.transform(X_test_imputed)

# Step 5: Initializing and training the Logistic Regression classifier.
print("🧠 Training blind Logistic Regression (searching for hidden statistical relationships)...")
...
print(f"🏆 Blind Logistic Regression ACCURACY: {accuracy * 100:.2f}%")
print("="*50)
print("\n📋 Detailed Report:")
...
# This is where we take advantage of the X.columns that we saved intact from Step 3
...
plt.title('Discovery of Alternative Biomarkers for CKD\n(Blind Logistic Regression)', fontsize=14)
plt.xlabel('Coefficient Magnitude (Statistical Importance)', fontsize=12)
